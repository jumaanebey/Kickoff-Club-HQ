-- Fix enum issue by casting strings to course_category type
-- This is required because 'fundamentals', 'rules', etc are custom enum values

-- First, let's make sure the enum type exists and has the values we need
DO $$ 
BEGIN 
    -- We can't easily check enum values in a DO block without complex queries, 
    -- but we can try to add them if they don't exist (Postgres 12+)
    -- For now, we will assume the type exists as defined in the TypeScript types
    -- 'quarterback', 'wide_receiver', 'running_back', 'offensive_line', 'defense', 'special_teams', 'general'
    
    -- Wait, the TypeScript types show a mismatch with the seed data!
    -- TS Types: quarterback, wide_receiver, running_back, offensive_line, defense, special_teams, general
    -- Seed Data uses: fundamentals, rules, positions, strategy, defense, history
    
    -- WE NEED TO UPDATE THE SEED DATA TO MATCH THE ENUM TYPES
    -- Mapping:
    -- fundamentals -> general
    -- rules -> general
    -- positions -> general (or specific if applicable)
    -- strategy -> general
    -- defense -> defense
    -- history -> general
END $$;
...
-- 4. Offense -> offensive_line (closest match for strategy?) or general
(
