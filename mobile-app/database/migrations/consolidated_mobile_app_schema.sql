-- Consolidated Mobile App Database Schema Migration
-- Run this in your Supabase SQL Editor to add all missing mobile app features
-- This includes: Knowledge Points, User Buildings, Daily Missions, Training Drills, Squad Units, Seasons
...
-- Mission templates (predefined challenges)
...
CREATE POLICY "Service can insert user missions"
...
-- Function to assign daily missions (3 common, 1 rare, maybe 1 epic/legendary)
...
DECLARE
  ...
  v_epic_missions UUID[];
  v_legendary_missions UUID[];
  v_assigned_missions JSON[] := '{}';
  v_mission UUID;
...
BEGIN
  ...
  -- Get available mission templates by rarity
  SELECT ARRAY_AGG(id) INTO v_common_missions
  FROM mission_templates WHERE rarity = 'common';

  SELECT ARRAY_AGG(id) INTO v_rare_missions
  FROM mission_templates WHERE rarity = 'rare';

  SELECT ARRAY_AGG(id) INTO v_epic_missions
  FROM mission_templates WHERE rarity = 'epic';

  SELECT ARRAY_AGG(id) INTO v_legendary_missions
  FROM mission_templates WHERE rarity = 'legendary';

  -- Assign 3 random common missions
  FOR i IN 1..3 LOOP
    v_mission := v_common_missions[1 + floor(random() * array_length(v_common_missions, 1))];

    INSERT INTO user_missions (user_id, template_id, expires_at)
    VALUES (p_user_id, v_mission, v_expires_at)
    ON CONFLICT DO NOTHING;
  END LOOP;

  -- Assign 1 rare mission
  v_mission := v_rare_missions[1 + floor(random() * array_length(v_rare_missions, 1))];
  INSERT INTO user_missions (user_id, template_id, expires_at)
  VALUES (p_user_id, v_mission, v_expires_at)
  ON CONFLICT DO NOTHING;

  -- 10% chance for epic mission
  ...
  -- 1% chance for legendary mission
...
BEGIN
  ...
  SET progress = LEAST(progress + p_increment, mt.target_value),
      is_completed = (progress + p_increment >= mt.target_value)
  FROM mission_templates mt
  WHERE um.template_id = mt.id
...
BEGIN
  SELECT user_id, is_completed, is_claimed
  INTO v_user_id, v_is_completed, v_is_claimed
  FROM user_missions
  ...
  SELECT mt.coin_reward, mt.xp_reward
  INTO v_coin_reward, v_xp_reward
  ...
  SELECT v_user_id, v_coin_reward, 'Mission Reward', coins
...
-- ============================================================================
-- 3. SQUAD UNITS & TRAINING SYSTEM
-- ============================================================================

-- User squad units (players they own and train)
...
-- Training sessions (FarmVille-style timers)
...
-- User seasons (track season progress)
...
CREATE POLICY "Users can update their own squad"
  ON user_squad_units FOR ALL
  USING (auth.uid() = user_id);

-- RLS for training sessions
...
CREATE POLICY "Users can manage their own seasons"
  ON user_seasons FOR ALL
  USING (auth.uid() = user_id);

-- RLS for match results
...
CREATE POLICY "Users can insert their own match results"
  ON user_match_results FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Function to initialize user squad units (called on signup)
...
AS $$
BEGIN
  -- Create starter units for all positions
  INSERT INTO user_squad_units (user_id, unit_type, overall_rating, attack_stat, defense_stat, speed_stat)
  VALUES
  ...
  INSERT INTO user_seasons (user_id, season_number, is_active)
...
DECLARE
  v_session_id UUID;
  v_stat_increase INTEGER;
BEGIN
  -- Calculate stat increase based on duration
  v_stat_increase := CASE
    WHEN p_duration_minutes = 15 THEN 1
    WHEN p_duration_minutes = 30 THEN 3
    WHEN p_duration_minutes = 60 THEN 6
...
BEGIN
  SELECT user_id, unit_type, stat_increase, claimed, completes_at
  INTO v_user_id, v_unit_type, v_stat_increase, v_claimed, v_completes_at
  FROM training_sessions
  WHERE id = p_session_id;

  IF v_claimed THEN
    RAISE EXCEPTION 'Training session already claimed';
  END IF;

  IF v_completes_at > NOW() THEN
    RAISE EXCEPTION 'Training session not yet complete';
  END IF;

  -- Update unit stats
  UPDATE user_squad_units
  SET overall_rating = LEAST(overall_rating + v_stat_increase, 99),
      training_points = training_points + v_stat_increase
  WHERE user_id = v_user_id AND unit_type = v_unit_type;
...
DECLARE
  ...
  v_season_id UUID;
BEGIN
  -- Calculate user's team rating
  SELECT AVG(overall_rating)::INTEGER INTO v_user_rating
  FROM user_squad_units
  WHERE user_id = p_user_id;

  -- Generate opponent rating (60-80 range)
  v_opponent_rating := 60 + floor(random() * 20);

  -- Simulate scores based on ratings
  ...
  SET games_played = games_played + 1,
      games_won = games_won + CASE WHEN v_result = 'win' THEN 1 ELSE 0 END,
      games_lost = games_lost + CASE WHEN v_result = 'loss' THEN 1 ELSE 0 END,
      points_for = points_for + v_user_score,
      points_against = points_against + v_opponent_score
...
SELECT
  'knowledge_point_transactions' as table_name,
  COUNT(*) as row_count
FROM knowledge_point_transactions
UNION ALL
...
UNION ALL
SELECT 'user_squad_units', COUNT(*) FROM user_squad_units
UNION ALL
SELECT 'training_sessions', COUNT(*) FROM training_sessions
UNION ALL
SELECT 'user_seasons', COUNT(*) FROM user_seasons
UNION ALL
SELECT 'user_match_results', COUNT(*) FROM user_match_results;
